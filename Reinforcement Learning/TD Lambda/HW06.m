...
% initial parameteres
x_size= 15; y_size= 15; n_iteration= 1500; n_trial= 3004;
learning_rate= 1; discount_factor= 1; lambda= 0.9;
...
cat_cordinate= [12 4];
x_mouse_range= [1:4,6:11,13:15];
y_mouse_range= [1:3,5:9,12:15];
...
% GameBoard(x_mouse(1),y_mouse(1),platform_cordinate,cat_cordinate,d_mouse1);
    ...
    for i= 1:n_iteration-1
        if x_mouse(j,i) ~= cat_cordinate(1) || y_mouse(j,i) ~= cat_cordinate(2)
            if x_mouse(j,i) ~= platform_cordinate(1) || y_mouse(j,i) ~= platform_cordinate(2)
                ...
                state_value= TD_function(old_state,state_value,x_mouse(j,:),y_mouse(j,:),lambda,i);
                ...
                % uncomment next line.
                %GameBoard(x_mouse(j,i+1),y_mouse(j,i+1),platform_cordinate,cat_cordinate,d_mouse1);
                ...
                % redesign game board
                %GameBoard(x_mouse(j,i),y_mouse(j,i),platform_cordinate,cat_cordinate,d_mouse3);
            ...
            % redesign game board
            %GameBoard(x_mouse(j,i),y_mouse(j,i),platform_cordinate,cat_cordinate,d_mouse2);
            %plot(x_mouse(j,1:i),y_mouse(j,1:i),'r','linewidth',1);
            %drawnow
            %             if mod(i,20)==0
            %                 % get this frame
...
% initial parameteres
x_size= 15; y_size= 15; n_iteration= 1500; n_trial= 2000; lambda= 0.9;
